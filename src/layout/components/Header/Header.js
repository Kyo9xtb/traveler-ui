...
function Header() {
    ...
    useEffect(() => {
        ...
        $(`.${cx('menu-item-has-children')}`).hover(
            function () {
        ...
        <header className={cx('header')}>
                        ...
                        <div className={cx('header-contact', 'text-start')}>
                            <Link to='#'>
                                ...
                                <div className={cx('header-contact-details', 'd-none d-sm-block')}>
                                    <span className={cx('contact-label')}>
                                        Hotline
                                    </span>
                                    <h5>
                                        0126 922 0162
                                                                        ...
                                                                        <div className={cx('sub-cate-1')}>
                                                                            <div className={cx('row', 'fix-padding')}>
                                                                                <div className='col-lg-9'>
                                                                                    <ul className={cx('nav-level1', 'row', 'active')}>
                                                                                        {menuItem.children.map((child, index) => {
                                                                                            return (
                                                                                                child.children !== undefined && child.children.length > 0 ?
                                                                                                    <li key={index} className={cx('level2', 'col-lg-4')}>
                                                                                                        <Link to={child.path} title={child.title}>{child.title}</Link>
                                                                                                        <ul className={cx('level3')}>
                                                                                                            {
                                                                                                                child.children.map((grandChild, index) => {

                                                                                                                    return (
                                                                                                                        <li key={index}>
                                                                                                                            <Link to={grandChild.path} title={grandChild.title}>{grandChild.title}</Link>
                                                                                                                        </li>
                                                                                                                    )
                                                                                                                })
                                                                                                            }
                                                                                ...
                                                                                </div>
                                                                                {
                                                                                    menuItem.banner !== undefined ?
                                                                                        <div className='col-lg-3'>
                                                                                            <Link to='#' title=''>
                                                                                                <img src={menuItem.banner} alt={menuItem.title} />
                                                                                            </Link>
                                                                                        </div> : <Fragment />
                                                                                }
                                                                            </div>
            ...
            <div className={cx('mobile-menu-container')}>
                <div className={cx('slicknav_menu')}>
                    <Link className={cx('slicknav_btn')} onClick={handleMenuMobile}>Menu</Link>
                </div>
                            ...
                            return (
                                menu.children !== undefined && menu.children.length > 0 ?
                                    <li key={index} className={cx('ng-scope', 'ng-has-child1')}>
                                        <Link to={menu.path}>
                                            {menu.title}
                                            <FontAwesomeIcon icon={faChevronUp} exact="true" onClick={handleMenuChildrenMobile} />
                                        </Link>
